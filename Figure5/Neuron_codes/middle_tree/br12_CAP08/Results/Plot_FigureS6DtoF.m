...
%% load data obtained from Neuron code corresponding to Branch12.hoc
sn='003_001_012_';
vtip_mat = [];
vprox_mat = [];
vmid_mat =[];
for iter = iter_list
  vtip = load([sn num2str(iter,'%03.f') '_001_vbr12_dist.dat']);
   vtip_mat=[vtip_mat vtip];
   vmid = load([sn num2str(iter,'%03.f') '_001_vbr12_mid.dat']);
   vmid_mat=[vmid_mat vmid];
   vprox = load([sn num2str(iter,'%03.f') '_001_vbr12_prox.dat']);
   vprox_mat=[vprox_mat vprox];
end

figure(1)
plot(t,vtip_mat(:,35),'Color','[0 0.4470 0.7410]','LineWidth',2.5)
hold on
plot(t,vtip_mat(:,36),'Color','[0.8500 0.3250 0.0980]','LineWidth',2.5)
hold on
plot(t,vtip_mat(:,37),'Color','0.9290 0.6940 0.1250','LineWidth',2.5)
hold on
...
legend('70PF','72PF','74PF','80PF','90PF','100PF')
hgexport(gcf, 'FigureS6E', hgexport('factorystyle'), 'Format', 'png','Resolution','1000')

figure(2)
plot(t,vmid_mat(:,35),'Color','[0 0.4470 0.7410]','LineWidth',2.5)
hold on
plot(t,vmid_mat(:,36),'Color','[0.8500 0.3250 0.0980]','LineWidth',2.5)
hold on
plot(t,vmid_mat(:,37),'Color','0.9290 0.6940 0.1250','LineWidth',2.5)
hold on
...
xticklabels({'-10','-5','0','5','10','15','20','25','30'})
legend('70PF','72PF','74PF','80PF','90PF','100PF')
hgexport(gcf, 'FigureS6extra', hgexport('factorystyle'), 'Format', 'png','Resolution','1000')


figure(3)
plot(t,vprox_mat(:,35),'Color','[0 0.4470 0.7410]','LineWidth',2.5)
hold on
plot(t,vprox_mat(:,36),'Color','[0.8500 0.3250 0.0980]','LineWidth',2.5)
hold on
plot(t,vprox_mat(:,37),'Color','0.9290 0.6940 0.1250','LineWidth',2.5)
hold on
...
xticklabels({'-10','-5','0','5','10','15','20','25','30'})
legend('70PF','72PF','74PF','80PF','90PF','100PF')
hgexport(gcf, 'FigureS6F', hgexport('factorystyle'), 'Format', 'png','Resolution','1000')
...
grid on
hold on
plot(72,10,'color','k','Marker','hexagram','Markersize',20,'MarkerFaceColor','k')
set(gca,'FontSize',20)
...
hgexport(gcf, 'FigureS6D', hgexport('factorystyle'), 'Format', 'png','Resolution','1000')

save('br12_CAP08_new.mat','nb_syn','v_peak_vtip','v_peak_vmid','v_peak_vprox')
